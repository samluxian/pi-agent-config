...
| GitLab pipeline failure with a known pipeline ID | One delivery stage failed before later stages could run | `diagnostics_flow.sh gitlab-pipeline <project> <pipeline-id>` | Failed stage/job and compact key lines identify pre-check, build, test/package, GitOps update, legacy deploy, or downstream; missing trace/auth is an inconclusive validation gap | Stop at the first failing layer; for source/build failure hand off the job, revision, compact error, and named file without crossing to desired/live state; read more trace only when compact lines are inconclusive |
| GitLab batch trigger appears successful but no update is observed | A batch update/deploy job was skipped or failed, or its pipeline source differs from the rule assumption | `diagnostics_flow.sh gitlab-batch-pipelines [repository-path]` | The probe resolves the local `origin`, verifies the exact GitLab project through `glab`, then starts with exact `update-image-tag-batch` job history and reads each owning pipeline's metadata and batch job summary. `skipped` is a warning, not proof of cause. An empty history, remote parse, authentication, project-access, or API failure is inconclusive. | Treat vendor documentation and static CI rules as a hypothesis. Stop after one bounded scan when target job/pipeline metadata establishes the source; otherwise request a controlled user-operated event. Use the reported pipeline ID with `gitlab-pipeline` only when a specific trace is needed. |

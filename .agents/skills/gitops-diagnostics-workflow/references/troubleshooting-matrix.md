...
| Runtime error in a running container | Application/dependency failure rather than Kubernetes startup failure | Time-bounded `kubectl logs ... --tail=100 --since=<window>` or filtered error tail | A concrete error names the next application/dependency check; a window outside retained logs is `no coverage`; empty or unrelated lines are inconclusive | Stop on a sufficient application cause; route to runtime dependency checks only when the error names one |
| GitLab pipeline failure | One delivery stage failed before later stages could run | `diagnostics_flow.sh gitlab-pipeline <project> <pipeline-id>` | Failed stage/job and compact key lines identify pre-check, build, GitOps update, legacy deploy, or downstream; missing trace/auth is an inconclusive validation gap | Stop at the first failing delivery layer; read more trace only when compact lines are inconclusive |

...
| Context-window retrospective | Manual `/skill:context-window-retrospective` README entry with no automatic invocation fixtures; active-branch latest-compaction boundary; bounded metadata only; abandoned branch excluded; semantic reviewer verdict counts; initiative follow-up signals remain review candidates rather than proof; necessary approval, authentication, scope, secret, and mutation stops are excluded; no prompt, command output, diff, log, or secret content |
...
| Contract validator implementation | `npm run test:contract-unit`; current-repository structural run; error output remains bounded |
| `AGENTS.md` | Within context budget; README synchronized; shared rule is always-on; explicit approval forces a fresh target-repository branch, working-tree, staged, and unstaged check before branch-safety decisions or edits; skills-repository main exception covers explicitly requested repository-owned files; a named presentation-repository main/master exception requires explicit approval, a clean tree, tracked slide-owned files, and excludes other target repositories, dependencies/configuration, secrets, generated/git-ignored files, and Git/remote mutation; no skill routing table or domain workflow; `git diff --check` |

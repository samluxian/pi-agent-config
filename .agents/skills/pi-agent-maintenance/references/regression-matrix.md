...
| Subagent extension | Parent/child authority boundary; execution-capable reviewer without write/edit; reviewer may delegate bounded single/parallel repository reading only to leaf scouts while retaining commands and verdict; repository-scoped parent/worker review gate; semantic pass versus blocked/fail/missing verdict; partial/final behavior; stale/failed/reset behavior; profile/tool allowlist; role single/parallel limits; structured timeout/abort/process failure; bounded reviewer output; no paid provider call in unit tests |
| Cross-skill orchestration | `AGENTS.md` companion entry; one final reviewer per changed repository; at most three independent heavy units; changed-path-first validation; worker review reuse; domain evidence and stop conditions retained; orchestrator owns routing, prompts, concurrency, authority, and reconciliation; no workflow copied into domain skills |
...
| Public repository safety | No private company/client identifiers in current paths or text; placeholders use reserved examples; generic scanner passes; machine-local terms are not printed or stored; final reviewer checks proper nouns semantically; history remains a separate publication gate |
